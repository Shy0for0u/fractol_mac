...
/*   Updated: 2019/08/05 20:38:26 by dgorold-         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
void			regular_works(t_fcl *fcl)
{
	...
	mlx_destroy_image(fcl->mlx_ptr, fcl->img.img_pntr);
}
...
int				deal_key(int key, t_fcl *fcl)
{
	if (key == 53)
		exit(0);
	if (key == 46)
		(fcl->ms.stopmove) ? (fcl->ms.stopmove = 0) : (fcl->ms.stopmove = 1);
...
int 			mouse_move(int x, int y, t_fcl *fcl)
{
	...
	fcl->img.data = mlx_get_data_addr(fcl->img.img_pntr,
									  &fcl->img.bpp, &fcl->img.s_l, &fcl->img.endian);
	if ((x < 0 || x >= WIDTH || y < 0 || y >= HEIGHT) && fcl->ms.stopmove == 0)
	{
		if (x > WIDTH)
			x = WIDTH;
		if (x > HEIGHT)
